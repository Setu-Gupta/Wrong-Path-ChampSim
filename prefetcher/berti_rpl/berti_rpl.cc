...
/******************************************************************************/
uint8_t LatencyTable::add(uint64_t addr, uint64_t tag, bool pf,
                          uint64_t cycle) {
  /*
...
/******************************************************************************/
bool ShadowCache::add(uint32_t set, uint32_t way, uint64_t addr, bool pf,
                      uint64_t lat) {
  /*
...
void HistoryTable::add(uint64_t tag, uint64_t addr, uint64_t cycle) {
  ...
  if constexpr (champsim::debug_print) {
    ...
    std::cout << " tag: " << std::hex << tag << " line_addr: " << addr
              << std::dec;
...
uint16_t HistoryTable::get_aux(uint32_t latency, uint64_t tag,
                               uint64_t act_addr, uint64_t *tags,
                               uint64_t *addr, bool *on_time, uint64_t cycle) {
  /*
  ...
  do {
    ...
    if (pointer->tag == tag && pointer->time <= cycle) {
      ...
      on_time[num_on_time] = true;
      num_on_time++;
    } 
    else if (pointer->tag == tag){
      tags[num_on_time] = pointer->tag;
      addr[num_on_time] = pointer->addr;
      on_time[num_on_time] = false;
      num_on_time++;
    }
...
uint16_t HistoryTable::get(uint32_t latency, uint64_t tag, uint64_t act_addr,
                           uint64_t *tags, uint64_t *addr, bool *on_time,
                           uint64_t cycle) {
  /*
   * Return an array (by parameter) with all the possible PC that can launch
   * an on-time and late prefetch
  ...
  uint16_t num_on_time =
      get_aux(latency, tag, act_addr, tags, addr, on_time, cycle & TIME_MASK);
...
void Berti::increase_conf_tag(uint64_t tag) {
  ...
  if (bertit[tag]->conf == CONFIDENCE_MAX) {
    ...
    for (auto &i : bertit[tag]->deltas) {
      ...
      // Check if we have to update the replacement policy
      //if (i.rpl_conf > CONFIDENCE_RPL) {
      //if (i.rpl_conf == CONFIDENCE_RPL_EXACT){
      if (i.rpl_conf >= CONFIDENCE_RPL_FLOOR && i.rpl_conf <= CONFIDENCE_RPL_CEIL){
        i.up_rpl = BERTI_RPL;
      } else {
        i.up_rpl = BERTI_R;
      }
      ...
      i.rpl_conf = 0; // Amdiii, reset rpl confidence
...
void Berti::add(uint64_t tag, int64_t delta, bool on_time) {
  /*
   * Save the new information into the history table
   *
   * Parameters:
  ...
  auto add_delta = [](auto delta, auto entry) {
    ...
    auto it = std::find_if(std::begin(entry->deltas), std::end(entry->deltas),
                           [](const auto i) { return (i.delta == 0); });
  ...
  for (auto &i : entry->deltas) {
    if (i.delta == delta) {
      ...
      i.rpl_conf += CONFIDENCE_INC;
      //Amdiii
      if (i.rpl_conf > CONFIDENCE_RPL_MAX)
        i.rpl_conf = CONFIDENCE_RPL_MAX;
  ...
  auto ssize = std::count_if(std::begin(entry->deltas), std::end(entry->deltas),
                             [](const auto i) { return i.delta != 0; });
  ...
  if (entry->deltas.front().rpl == BERTI_R ||
      entry->deltas.front().rpl == BERTI_L2R) {
    if constexpr (champsim::debug_print)
      std::cout << " replaced_delta: " << entry->deltas.front().delta
                << std::endl;
    entry->deltas.front().delta = delta;
    if (on_time)
      entry->deltas.front().conf = CONFIDENCE_INIT;
    else
      entry->deltas.front().conf = 0;
    entry->deltas.front().rpl_conf = CONFIDENCE_INIT;
  }
}
...
void Berti::find_and_update(uint64_t latency, uint64_t tag, uint64_t cycle,
  ...
  bool on_time[HISTORY_TABLE_WAYS]; // This delta is on_time
  uint16_t num_on_time = 0;

  // Get the IPs that can launch a prefetch
  num_on_time =
      historyt->get(latency, tag, line_addr, tags, addr, on_time, cycle);
  bool increaseConf = false;
  ...
  for (uint32_t i = 0; i < num_on_time; i++) {
    ...
    if ((std::abs(stride) < (1 << DELTA_MASK)) && stride != 0)
      add(tags[i], stride, on_time[i]);
  }
}
...
bool Berti::compare_rpl(delta_t a, delta_t b) {
  if (a.rpl == BERTI_R && b.rpl != BERTI_R)
    ...
    //if (a.up_rpl == BERTI_R && b.up_rpl == BERTI_RPL)
    //  return 1;
    //else if (a.up_rpl != BERTI_R && b.up_rpl == BERTI_R)
    //  return 0;
...
void CACHE::prefetcher_initialize() {
  ...
  for (auto const &i : get_rq_size())
    latency_table_size += i;
  for (auto const &i : get_wq_size())
    latency_table_size += i;
  for (auto const &i : get_pq_size())
    latency_table_size += i;
  ...
  std::cout << "Berti RPL Prefetcher" << std::endl;

  std::cout << "Confidence: " << std::endl;
  std::cout << "  L1D: " << CONFIDENCE_L1 << std::endl;
  std::cout << "  L2D: " << CONFIDENCE_L2 << std::endl;
  std::cout << "  L2DR: " << CONFIDENCE_L2R << std::endl;
  //std::cout << "  RPL: " << CONFIDENCE_RPL_EXACT << std::endl;
  std::cout << "  RPL: " << CONFIDENCE_RPL_FLOOR << " to " << CONFIDENCE_RPL_CEIL << std::endl;
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip,
  ...
  } else if (cache_hit && scache->is_pf(line_addr))          // Hit bc prefetch
  {
    ...
    berti->find_and_update(latency, ip_hash, current_cycle & TIME_MASK,
                           line_addr);
  ...
  for (auto i : deltas) {
    ...
    if (latencyt->get(p_b_addr))
      continue;
    if (i.up_rpl == BERTI_R && i.rpl == BERTI_R)
      continue;
    if (p_addr == 0)
      continue;
    ...
    // Amdiii PQ occupancy ratio limit
    /*if (i.rpl == BERTI_R && i.up_rpl != BERTI_R){
        if ((float)get_pq_occupancy()[2]/get_pq_size()[0] > PQ_LIMIT){
		continue;
	}
    }*/


    // if (i.rpl == BERTI_R) continue; // Amdiii: Uncomment to get back to berti baseline
    ...
    // Amdiii Debug
    //if (i.rpl == BERTI_R && i.up_rpl != BERTI_R)
	//    std::cout << i.rpl_conf << std::endl;

    // Amdiii
    // metadata = (pf,rpl)
    // send request to update rpl (no pf, but update rpl)
    if (i.rpl == BERTI_R && i.up_rpl != BERTI_R){
      pf_to_rpl++;
      metadata = 0x01;
    }
    // pf & update rpl
    else if (i.rpl != BERTI_R)
    //else if (i.rpl != BERTI_R && i.up_rpl != BERTI_R)
      metadata = 0x11;
    ...
    if (prefetch_line(p_addr, fill_this_level, metadata)) // Send the info if we should update or not rpl
    {
      ++average_issued;
      if (first_issue) {
        first_issue = false;
    ...
    if (latency != 0) {
      ...
      if (average_latency.num == 0)
        ...
        average_latency.average =
            average_latency.average +
            ((((float)latency) - average_latency.average) /
             average_latency.num);
      }
      average_latency.num++;
    }
...
void CACHE::prefetcher_final_stats() {
  ...
  std::cout << "AMDIII TO_RPL: " << pf_to_rpl << std::endl;
