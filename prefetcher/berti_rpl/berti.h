...
typedef struct Delta {
  ...
  uint8_t rpl;
  uint8_t up_rpl;
  uint64_t rpl_conf; // This is the the confidence to be replaced
  Delta() : conf(0), delta(0), rpl(BERTI_R), up_rpl(BERTI_R), rpl_conf(0) {};
} delta_t;
...
public:
  ...
  uint16_t get(uint32_t latency, uint64_t tag, uint64_t act_addr,
               uint64_t *tags, uint64_t *addr, bool *on_time, uint64_t cycle);
};
...
private:
  ...
  void add(uint64_t tag, int64_t delta, bool on_time);
