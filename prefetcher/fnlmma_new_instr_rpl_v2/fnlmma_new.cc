...
#define FNL_RPL_THRESHOLD 3     // WorthPF should be higher than this value for prefetching; otherwise candidate is used as a replacement hint
#define MMA_RPL_THRESHOLD 1     // Consecutive count should be higher than this value for prefetching; otherwise candidate is used as a replacement hint
...
public:
  ...
  std::pair<uint64_t, int8_t> AheadPredict (uint64_t Addr)
  {
    PrefetchCandidate = 0;
...
//if Block B-1 was accessed recently one has only to prefetch Block block+FNL
		  if ((WasNotJustFnl (Block)) || (i == MAXFNL))
		    {                            
                        if(WorthPF[index] >= FNL_RPL_THRESHOLD)
                        {
                                prefetch_line ((pf_Block)<<LOG2_BLOCK_SIZE, true, PF_EN | UP_RPL_EN);
                        }
                        else
                        {
                                prefetch_line ((pf_Block)<<LOG2_BLOCK_SIZE, true, UP_RPL_EN);
                        }
...
/////
      ...
      auto prediction = AHEADphist.AheadPredict ((v_addr >> 2) ^ (PREVADDR[NSHIFT - 1] << 1));
      AheadPredictedBlock = prediction.first;
      AheadPredictConf = prediction.second;
...
// avoid issing prefetch the block if the previous block was prefetched
              ...
              if(AheadPredictConf >= MMA_RPL_THRESHOLD)
              {
                        prefetch_line ((AheadPredictedBlock)<<LOG2_BLOCK_SIZE, true, PF_EN | UP_RPL_EN);
              }
              else
              {
                        prefetch_line ((AheadPredictedBlock)<<LOG2_BLOCK_SIZE, true, UP_RPL_EN);
              }
...
//First access resulted in a miss
      if (AheadPredictedBlock == 0)
	{
	  ...
	  auto prediction = AHEAD.AheadPredict (v_addr >> 2);
          AheadPredictedBlock = prediction.first;
          AheadPredictConf = prediction.second;
